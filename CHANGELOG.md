...
### Changed — полное описание в постах
- 📝 Пост показывает **полное описание события** (до 1500 символов) вместо обрезанных 300
- 🗄 Миграция 003: `events.short_description` `String(1024)` → `Text` (описания источников достигают 3000)
- 🔄 `SHORT_DESC_LIMIT` / `POST_DESC_LIMIT` → 1500 (покрывает ~95% описаний)

### Verified
- ✅ Событие «Звуки и свечи. Рок-баллады» (1759 симв.) — полное описание в посте
- ✅ 172 теста проходят
