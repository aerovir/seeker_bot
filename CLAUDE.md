...
## Исследование ошибок (EVIDENCE.md)

1. **Каждое расследование ошибки документируется** в `EVIDENCE.md`: симптом → исследование → корень → фикс/статус, с датой.
2. **Если корень НЕ подтверждён в коде/логах/данных** — запись помечается как **⚠️ Гипотеза, требующая ручного внимания**. Не выдавать гипотезу за установленный факт.
3. **Подтверждённое** помечается ✅ и указывает конкретный код/логи/данные, на которых основан вывод.
4. Правило действует независимо от того, удалось ли найти фикс: неуспешные расследования тоже документируются (фикс может быть найден позже).
